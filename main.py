...
st.set_page_config(page_title="IA Rei da Bola: Estrategista", page_icon="🏆", layout="wide")

st.title("🏆 IA Rei da Bola: Estrategista Pro")
st.markdown("---")
...
if st.button("🔥 SCANEAR OPORTUNIDADES"):
    ...
    if res.get('response'):
        for j in res['response']:
            ...
            # --- LÓGICA SEPARADA ---
            # IG (Índice de Gols): Foco em pontaria
            ig = (no_alvo * 8) + (ataques_p / 5)
            # IC (Índice de Cantos): Foco em volume e chutes totais (mesmo que fora)
            ...
            with st.container():
                ...
                with c1:
                    ...
                    st.caption(f"⏰ {tempo}' min | 🚩 Cantos: {escanteios}")
                    st.write(f"🎯 No Alvo: {no_alvo} | 👟 Chutes Totais: {no_alvo + fora_alvo}")
                
                with c2:
                    if ig > 40: st.success(f"⚽ ALTO RISCO DE GOL (IG: {ig:.1f})")
                    if ic > 35: st.warning(f"🚩 PRESSÃO PARA CANTOS (IC: {ic:.1f})")
                    if ig <= 40 and ic <= 35: st.info("⚖️ Jogo Estudado")
                
                with c3:
                    # VEREDITO COM BASE NA SEPARAÇÃO
                    if ic > 40 and tempo > 75:
